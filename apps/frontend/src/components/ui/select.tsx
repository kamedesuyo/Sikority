  ...
  <SelectPrimitive.Portal>
    ...
    >
      <SelectPrimitive.Viewport
        className={["p-1", position === "popper" && "h-[var(--radix-select-trigger-height)] w-full min-w-[var(--radix-select-trigger-width)]"].filter(Boolean).join(" ")}
      >
        {children}
      </SelectPrimitive.Viewport>
    </SelectPrimitive.Content>
...
>(({ className, ...props }, ref) => (
  <SelectPrimitive.Label
    ref={ref}
    className={["py-1.5 pl-8 pr-2 text-sm font-semibold", className].filter(Boolean).join(" ")}
    {...props}
  />
));
